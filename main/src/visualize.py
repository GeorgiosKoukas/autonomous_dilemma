...
def draw_net(config, genome, view=False, filename=None, node_names=None, show_disabled=True, prune_unused=False,
    ...
    # Create subgraphs for layer separation
    with dot.subgraph(name='cluster_input') as c:
        c.attr(color='lightblue')
        c.attr(label='Input Layer')
        for k in config.genome_config.input_keys:
            name = node_names.get(k, str(k))
            c.node(name, _attributes={'style': 'filled', 'shape': 'box', 'fillcolor': node_colors.get(k, 'lightgray')})
    ...
    with dot.subgraph(name='cluster_output') as c:
        ...
        for k in config.genome_config.output_keys:
            name = node_names.get(k, str(k))
            c.node(name, _attributes={'style': 'filled', 'fillcolor': node_colors.get(k, 'lightblue')})
