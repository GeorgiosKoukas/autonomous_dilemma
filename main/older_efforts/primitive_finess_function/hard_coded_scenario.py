...
NUM_LANE_1, NUM_LANE_2 = 5, 3   #***********************
pedestrian_data = pd.read_csv('trolley.csv')
GENERATION_PEDESTRIAN_ATTRIBUTES = generation_pedestrian_attributes = pedestrian_data.sample(NUM_LANE_1 + NUM_LANE_2).to_dict('records')
...
class TrolleyScenario:
    def __init__(self, number_lane_1, number_lane_2, client, weather, pre_sampled_attributes, generation_spawn_locations):
        ...
        self.offset_1 = carla.Vector3D(2.5, 10, 0) #y is the distance facing the ego, x is the distance perpendicular to the ego
        self.offset_2 = carla.Vector3D(-2.5, 20, 0)
    ...
    def setup_variables(self, number_lane_1, number_lane_2, client):
        ...
        self.radius_x, self.radius_y = 1, 1
...
def eval_genomes(genomes, config):
    ...
    scenario_attributes = NUM_LANE_1, NUM_LANE_2, client, weather_params
    ...
    for genome_id, genome in genomes:
        ...
        scenario = TrolleyScenario(*scenario_attributes, GENERATION_PEDESTRIAN_ATTRIBUTES, generation_spawn_locations)
        scenario.run(net)   


        # Use the results to determine the loss
        harm_score = scenario.get_scenario_results()
        loss = scenario.total_harm_score
    ...
    # total_fitness = sum(genome.fitness for _, genome in genomes)
    # average_fitness = total_fitness / len(genomes) if genomes else 0

    # # Step 3: Calculate relative fitness
    # for _, genome in genomes:
    #     # Example: Normalize by total fitness (avoid division by zero)
    #     genome.fitness = genome.fitness / total_fitness if total_fitness else 0
        
    #     # OR: Adjust by average fitness
    #     # genome.fitness = genome.fitness - average_fitness
