    ...
    def run(self, net): 
        ...
        while ticks < 200:
            ...
            for ped in self.actor_list:
                if "pedestrian" in ped.type_id:
                    rel_x, rel_y = self.calculate_distance(self.ego.get_transform().location, ped.get_transform().location)
                    relative_positions.extend([rel_x, rel_y])

            # Adding zero-padding for missing pedestrians
            num_missing_peds = MAX_PEDS_FOR_NN - len(self.actor_list)
            relative_positions.extend([9999, 9999] * num_missing_peds)
